# Pamela & Alfredo — dirección de diseño

- Estética editorial, elegante y moderna inspirada en la referencia enviada.
- Paleta obligatoria: azul marino `#202b3d`, azul grisáceo `#435875`, café cálido `#936a4d`, beige `#cbb79f` y marfil `#f5f3ee`.
- Tipografía serif de alto contraste para títulos y sans serif espaciada para información funcional.
- Composición vertical propia con fotografías en arcos, tarjetas de recintos escalonadas, bloques de color amplios y ausencia de ornamento floral recargado.
- Prioridad móvil: jerarquía clara, botones cómodos y datos esenciales legibles sin zoom.
- La confirmación usa modo `mixed`: guarda en el panel RSVP y continúa por WhatsApp.
- La galería debe ocultarse en modo portafolio salvo autorización explícita.
- La sección de vestimenta conserva todas las indicaciones, tonos reservados y aviso de niñeras; la franja de atuendos proporcionada por los novios debe mostrarse sin fondo.
- El control musical usa `public/invitations/boda-pamela-y-alfredo/audio/cancion-de-boda.mp3` y solo aparece cuando ese archivo existe y puede reproducirse.
